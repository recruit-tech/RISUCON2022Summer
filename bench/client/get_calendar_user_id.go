...
func (c *Client) GetCalendarUserId(ctx context.Context, userID string, date int64) (*model.GetCalendarUserIdResponse, error) {
	const endpoint = "GET /calendar/:user_id"

	req, err := c.ag.GET(fmt.Sprintf("/calendar/%s", userID))
	if err != nil {
		return nil, fails.Wrap(err, fails.BenchmarkerErrorCode)
	}

	q := url.Values{}
	q.Add("date", strconv.FormatInt(date, 10))
	...
	if res.StatusCode == http.StatusOK {
		var rb model.GetCalendarUserIdResponse
