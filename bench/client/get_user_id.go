...
func (c *Client) GetUserId(ctx context.Context, id string) (*model.GetUserIdResponse, error) {
	const endpoint = "GET /user/:id"

	req, err := c.ag.GET(fmt.Sprintf("/user/%s", id))
	if err != nil {
	...
	if res.StatusCode == http.StatusOK {
		var rb model.GetUserIdResponse
